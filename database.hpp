...
inline std::unique_ptr<odb::database> create_database(const std::string& user,
...
{
    unsigned int portNumber = std::stoul(port);
    std::unique_ptr<odb::database> database (new odb::pgsql::database (user, password, db, host, portNumber, extra_conninfo));
    return database;
}
#endif
#endif // DATABASE_HXX
